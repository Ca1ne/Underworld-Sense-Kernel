...
static unsigned char current_constrast = 0x20;
				  ...
				  unsigned char *txdata, int length)
{
	...
	if (i2c_transfer_retry(ov9665_client->adapter, msg, 1) < 0) {
		pr_info("[CAM]ov9665_i2c_txdata failed\n");
		return -EIO;
	}
			      ...
			      unsigned char *rxdata, int length)
{
	...
	if (i2c_transfer_retry(ov9665_client->adapter, msgs, 2) < 0) {
		pr_info("[CAM]ov9665_i2c_rxdata failed!\n");
		return -EIO;
	}
				...
				unsigned short raddr, unsigned char *rdata)
{
	...
	*rdata = buf[0];
	if (rc < 0)
		pr_info("[CAM]ov9665_i2c_read failed!\n");
	...
	unsigned char addr,unsigned char Data, unsigned char Mask)
{
	...
	if(rc < 0){
		pr_err("[CAM]ov9665 error : read i2c error\n");
		return rc;
	}
	temp = (temp&(~Mask))|Data;
	rc = ov9665_i2c_write(ov9665_client->addr, addr, temp, BYTE_LEN);
	if(rc < 0){
		pr_err("[CAM]ov9665 error : write i2c error\n");
		return rc;
	}
	return rc;
}
...
static int ov9665_set_sensor_mode(int mode)
{
	...
	case SENSOR_PREVIEW_MODE:
		...
		pr_info("[CAM]ov9665:sensor set mode: preview\n");
		...
		/*scaling*/
		/* Wayne: default enable contrast*/
		ov9665_i2c_write(ov9665_client->addr, 0x64, 0xa6, BYTE_LEN);
		...
		/* Wayne: set contrast in preview init */
		ov9665_i2c_write_mask(0xc7, 0x34, 0x34);
		ov9665_i2c_write_mask(0xc8, 0x04, 0x04);
		ov9665_i2c_write_mask(0x64, 0x26, 0x26);
		ov9665_i2c_write(ov9665_client->addr, 0xd0,
					current_constrast, BYTE_LEN);
		mdelay(400);/*skip 2 break frame */
		break;

	case SENSOR_SNAPSHOT_MODE:
		pr_info("[CAM]ov9665:sensor set mode: snapshot\n");
		...
		ov9665_i2c_write(ov9665_client->addr, 0x2b, 0x00, BYTE_LEN);
		/*scaling*/
		ov9665_i2c_write(ov9665_client->addr, 0x64, 0x26, BYTE_LEN);
...
static int ov9665_set_contrast(enum contrast_mode contrast_value)
{
	...
	case CAMERA_CONTRAST_N2:
		ov9665_i2c_write(ov9665_client->addr,
			0xd0, 0x08, BYTE_LEN);
		current_constrast = 0x08;
		break;
	case CAMERA_CONTRAST_N1:
		ov9665_i2c_write(ov9665_client->addr,
			0xd0, 0x10, BYTE_LEN);
		current_constrast = 0x10;
		break;
	case CAMERA_CONTRAST_D:
		ov9665_i2c_write(ov9665_client->addr,
			0xd0, 0x20, BYTE_LEN);
		current_constrast = 0x20;
		break;
	case CAMERA_CONTRAST_P1:
		ov9665_i2c_write(ov9665_client->addr,
			0xd0, 0x30, BYTE_LEN);
		current_constrast = 0x30;
		break;
	case CAMERA_CONTRAST_P2:
		ov9665_i2c_write(ov9665_client->addr,
			0xd0, 0x40, BYTE_LEN);
		current_constrast = 0x40;
		break;
	default:
		break;
	}
...
static int ov9665_sensor_init(const struct msm_camera_sensor_info *data)
{
	...
	uint16_t model_id;
	int rc = 0;
	pr_info("[CAM]ov9665_sensor_init_probe \n");
	...
	pr_info("[CAM]ov9665: model_id = 0x%x\n", model_id);
...
int ov9665_sensor_open_init(struct msm_camera_sensor_info *data)
{
	...
	if (!ov9665_ctrl) {
		pr_info("[CAM]ov9665_init failed!\n");
		rc = -ENOMEM;
		goto init_done;
	}
	...
	pr_info("[CAM]ov9665: ov9665_sensor_probe switch clk\n");
	...
	if (!rc)
		...
		pr_info("[CAM]GPIO(%d) request faile",data->sensor_pwd);
	...
	if (rc < 0) {
		pr_info("[CAM]ov9665_sensor_init failed!\n");
		goto init_fail;
	}
	/*set initial register*/
	rc = ov9665_reg_init();
	if (rc < 0) {
		pr_info("[CAM]ov9665_sensor_reg_init failed!\n");
		goto init_fail;
	}
init_done:
	return rc;
...
int ov9665_sensor_release(void)
{
	...
	if (ov9665_ctrl) {
		...
		if (!rc)
			...
			pr_info("[CAM]GPIO(%d) request faile",
...
{
	int ret ;
	pr_info("[CAM]ov9665:kobject creat and add\n");
	android_ov9665 = kobject_create_and_add("android_camera2", NULL);
	if (android_ov9665 == NULL) {
		pr_info("[CAM]ov9665_sysfs_init: subsystem_register " \
		"failed\n");
		ret = -ENOMEM;
		return ret ;
	}
	pr_info("[CAM]ov9665:sysfs_create_file\n");
	ret = sysfs_create_file(android_ov9665, &dev_attr_sensor.attr);
	if (ret) {
		pr_info("[CAM]ov9665_sysfs_init: sysfs_create_file " \
		"failed\n");
	...
	if (ret) {
		pr_info("[CAM]ov9665_sysfs_init: sysfs_create_file htcwc failed\n");
		kobject_del(android_ov9665);
	}

	ret = sysfs_create_file(android_ov9665, &dev_attr_node.attr);
	if (ret) {
		pr_info("[CAM]ov9665_sysfs_init: dev_attr_node failed\n");
		kobject_del(android_ov9665);
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	pr_info("[CAM]ov9665_probe succeeded!\n");
...
probe_failure:
	...
	ov9665_sensorw = NULL;
	pr_info("[CAM]ov9665_probe failed!\n");
	return rc;
}
				...
				struct msm_sensor_ctrl *s)
{
	...
	/*MCLK enable*/
	pr_info("[CAM]ov9665: MCLK enable clk\n");
	msm_camio_clk_rate_set(24000000);
	mdelay(100);
